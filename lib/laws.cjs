...
function classifyStructure(node, probs) {
  ...
  // BATCH first: a container-level signal is definitive regardless of the values
  // it flattens. `batch_size`>1, or an `answered_by` array with per-run
  // cost/latency aggregates (allebee/jevgrep "batch 20").
  ...
  // MULTI-LABEL, math-based: a categorical distribution over mutually-exclusive
  // outcomes CANNOT have two probabilities each > 0.5 (they would sum past 1). So
  // >=2 values above 0.5 is definitive multi-label evidence -- independent
  // per-option probabilities -- and needs no sibling. Non-circular: it never
  // fires on a real categorical fabrication like {0.2,0.1} (no value > 0.5).
  // Caught NanoJev's arcade frames {north:0.84,...,south:0.83} which carry no
  // `truth` sibling to key off.
  if (Object.values(probs).filter((v) => v > 0.5).length >= 2) return 'multilabel';
  // MULTI-LABEL, sibling-based: a multi-hot boolean map keyed like the distribution.
